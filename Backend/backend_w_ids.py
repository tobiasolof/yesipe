...
def new_choice(data):
    ...
    chosen_ingredients.append(data)
    # TODO: handle chosen ingredients by name
    for v in ingredients.values():
        if v["id"] == data:
            new_choice_string = v["name"]
    try:
        for s in search.similar_vectors(new_choice_string, n=5):
            if s[1] > 0.75:
                chosen_ingredients.append(ingredients[s[0]]["id"])
    except KeyError:
        pass
    print('Chosen ingredients so far: {}'.format(chosen_ingredients))
...
def clear():
    ...
    chosen_ingredients = []  # TODO: should not do it locally
    # Empty recipes
...
def done():
    ...
    for recipe in master:
        temp_score = 0
        recipe_ids = [ingredients[i]["id"] for i in recipe['ingredients']]
...
def generate_suggestions():
    ...
    for ingredient in ingredients.values():
        tot_sum = sum(freq_matrix[chosen_ingredients[i]][ingredient["id"]] for i in range(len(chosen_ingredients)))
        worst = min(freq_matrix[chosen_ingredients[i]][ingredient["id"]] for i in range(len(chosen_ingredients)))
        freq = ingredient["freq"]
        main_freq = ingredient["main_freq"]
        score = (main_freq + 1) * worst

        if score > top_suggestions[0]["score"] and ingredient["id"] not in chosen_ingredients \
    ...
    for ingredient in ingredients.values():
        if ingredient["freq"] < 100:
            tot_sum = sum(
                freq_matrix[chosen_ingredients[i]][ingredient["id"]] for i in range(len(chosen_ingredients)))
            worst = min(
                freq_matrix[chosen_ingredients[i]][ingredient["id"]] for i in range(len(chosen_ingredients)))
            freq = ingredient["freq"]
            main_freq = ingredient["main_freq"]
            ...
            if score > top_group_3[0]["score"] and ingredient["id"] not in chosen_ingredients \
...
def generate_positions():
    ...
    device_size_x, device_size_y = [s for s in db.child("deviceSize").get().each()]
    min_radius, max_radius = device_size_x/6, device_size_y/8
    while True:
        try:
            suggestions = db.child('suggestions').get().each()
...
if __name__ == "__main__":
    ...
    no_suggestions = 50
    # TODO: make chose_ingredients a set
    chosen_ingredients = []
    clear()
    ...
    while True:
        ...
        if c == "-c":
            ...
            new_choice(db.child('suggestions').child(str(c)).child('id').get().val())
